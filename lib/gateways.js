                ...
                var query = connection.query('SELECT idrecurso_telefono,nombre,columna,fila,codec,clave_registro,' +
                    ...
                    function(err, rows) {
                        if (err) {
                            ...
                            /** 20200710. NUevos Parámetros para lineas telefonicas */
                                recurso.telefonia.iDetLineaAB = rows[i].iDetLineaAB;
                                recurso.telefonia.iEnableNoED137 = rows[i].iEnableNoED137;
                                recurso.telefonia.itiporespuesta = rows[i].itiporespuesta;
                                recurso.telefonia.additional_uri_remota = rows[i].additional_uri_remota;
                                recurso.telefonia.additional_superv_options = rows[i].additional_superv_options;
                                recurso.telefonia.additional_itiporespuesta = rows[i].additional_itiporespuesta;

                                logging.Info("(gateways).getAll (selectDataTfno) ", configName, ipGtw, recurso.IdRecurso);
...
exports.getTfnoRes4Gateway = function getTfnoRes4Gateway(resourceId, f) {
    ...
    logging.Info("(gateways).getTfnoRes4Gateway ", resourceId);
    connection.connect(function(err) {
        if (err) {
            logging.Error("Error connention to 'U5K-G' database.");
            return;
            ...
            var querySel = connection.query('SELECT rt.idrecurso_telefono,rt.nombre,rt.codec,rt.clave_registro,rt.precision_audio,' +
                ...
                function(err, result) {
                    logging.Info("(gateways).getTfnoRes4Gateway", resourceId, err, result[0].nombre, result[0].iEnableNoED137);
                    logging.Trace("(gateways).getTfnoRes4Gateway", err, querySel.sql);
                    connection.end();
                    if (err)
                        return f({ error: err.message });
                    else
...
exports.insertTfnoRes4Gateway = function insertTfnoRes4Gateway(tr, f) {
    ...
    logging.Info("(gateways).insertTfnoRes4Gateway ", tr.nombre);
    var insertedResource = '';
    var activa = 0;
    connection.connect(function(err) {
        if (err) {
                ...
                function(err, rows) {
                    ...
                    /** 20200710. NUevos Parámetros para lineas telefonicas */
                        ...
                        /** 20200710. NUevos Parámetros para lineas telefonicas */
                            ...
                            function(err, result) {
                                logging.Trace(querySel.sql);
                                logging.Info("(gateways).insertTfnoRes4Gateway ", tr.nombre, err);
                                if (err == null) {
                                    insertedResource = result.insertId;
                                    var query = connection.query('UPDATE pasarelas p ' +
                                        'SET p.pendiente_actualizar=1 ' +
...
exports.updateTfnoRes4Gateway = function updateTfnoRes4Gateway(tr, resId, f) {
    ...
    logging.Info("(gateways).updateTfnoRes4Gateway ", tr.nombre);
    var activa = 0;
    connection.connect(function(err) {
        if (err) {
            logging.Error("Error connention to 'U5K-G' database.");
                ...
                function(err, rows) {
                    if (err) {
                        ...
                        /** 20200710. NUevos Parámetros para lineas telefonicas */
                            ...
                            function(err, result) {
                                logging.Trace(query.sql);
                                logging.Info("(gateways).updateTfnoRes4Gateway ", tr.nombre, err, result);
                                if (err == null) {
                                                ...
                                                var delQuery = connection.query('DELETE FROM rangos_ats WHERE recurso_telefonico_id=?',
                                                    ...
                                                    function(err, result) {
                                                        if (tr.ranks.length > 0) {
                                                            ...
                                                            if (err == null) {
                                                                /** 20201106. Todos los recursos tienen Rangos al menos de origen */
...
exports.copyGateway = function(sourceIdGateway, targetNameGateway,
    ...
    logging.Info("(gateways).copyGateway ", sourceIdGateway, targetNameGateway, ip0TargetGateway, ip1TargetGateway, ipvTargetGateway);
                ...
                for (var i = 0; i < oldGtwTfnoResData.length; i++) {
                    if (oldGtwTfnoResData[i].pasarela_id != null) {
                        ...
                        query = 'INSERT INTO recursos_telefono (pasarela_id,fila,columna,nombre,' +
                            ...
                            ');';
                        logging.Info("copyGateway TfnoRes", sourceIdGateway, i, oldGtwTfnoResData[i].nombre);
 ...
 * 
 * @param {any} connection
 * @param {any} sourceResourceId
 * @param {any} targetResourceId
 * @param {any} f
 */
