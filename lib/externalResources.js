...
///*			f(OUT)					*/
///*  REV 1.0.2 VMG					*/
///*  REV 1.1.1 AGL: 20201026. Control de Alias o URIS repetidas */
///************************************/
exports.postExternalResource = function(extResource, f) {
    ...
    if (extResource.id_recurso == -1) {
        ...
        queryComp = "SELECT * FROM recursos_externos WHERE uri = '" + extResource.uri + "' OR alias='" + extResource.alias + "';";
    }
    else {
    ...
    if (queryComp == "") {
        ...
        ug5kdb.Query(queryComp, null, function (err, rows) {
            if (rows.length > 0) {
                if (f) f({ error: "Alias o URI repetidos", lista_recursos: null });
            }
            else {
                ug5kdb.Query(query, param, function (err, rows) {
                    var resultado = {
