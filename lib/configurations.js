...
///*  REV 1.0.2 VMG				*/
///********************************/
// exports.getConfiguration_old = function getConfiguration(req, res, cfg) {
//     var mySql = require('mySql');
//     var connection = ug5kdb.GetDbConnection();/* mySql.createConnection({
// 	  host     : gcfg.Ulises.dbhost,
// 	  user     : gcfg.Ulises.dbuser,
...
//         var strQuery = 'SELECT DISTINCT c.idconfiguracion as idCFG, c.nombre as name, ' +
//             'c.descripcion as description, c.activa as activa, ' +
//             'DATE_FORMAT(c.activa, "%d/%m/%Y %H:%i:%s") as ts_activa, ' +
//             'e.nombre as nameSite, e.idemplazamiento as idEMPLAZAMIENTO, p.ultima_actualizacion ' +
...
// };
/**
 * 2019. Se cambia la query ya que debido a la selección DISTINCT de la fecha de ultima actualizacion, 
 * se generaban registros duplicados de emplazamientos que afectaban a la presentacion...
 * @param {any} req
 * @param {any} res
 * @param {any} cfg
 */
exports.getConfiguration = function getConfiguration(req, res, cfg) {
    ...
    connection.connect(function(err) {
        if (err) {
            ...
            /** Debido a la clausala GROUP esta query solo es compatible en MYSQL */
            var strQuery = 'SELECT c.idconfiguracion AS idCFG, c.nombre AS NAME, c.descripcion AS description, c.activa AS activa, ' +
                'DATE_FORMAT(c.activa, "%d/%m/%Y %H:%i:%s") AS ts_activa, e.nombre AS nameSite, ' +
...
///*  REV 1.0.2 VMG					*/
///************************************/
exports.putConfiguration_old = function putConfiguration(oldIdConf, cfg, f) {
    var mySql = require('mySql');
    var connection = ug5kdb.GetDbConnection();/* mySql.createConnection({
	  host     : gcfg.Ulises.dbhost,
	  user     : gcfg.Ulises.dbuser,
...
exports.putConfiguration = async function putConfiguration(oldIdConf, cfg, f) {
    var mySql = require('mySql');
    var connection = ug5kdb.GetDbConnection();

    /** 20190213 Actualizar la tabla de configuraciones y las pasarela solo si corresponde a una configuracion activa */
    var res = await ug5kdb.QuerySync('UPDATE configuraciones SET nombre=?, descripcion=? WHERE idconfiguracion=?',
        [cfg.name, cfg.description, cfg.idCFG]);
    if (!res.error) {
...
exports.copyConfiguration = function copyConfiguration(idSourceConfig, trgt, f) {
    ...
    logging.Info("copyConfiguration", idSourceConfig, trgt);
    connection.connect(function(err) {
        if (err) {
            logging.Error("Error connection to 'U5K-G' database.");
        }
            ...
            var insertNewGtws = async function /*insertNewGtws*/(callback) {
                var query = '';
                var res = '';
                var empId2Insert = 0;
                for (var i = 0; i < oldGtwData.length; i++) {
                    if (oldGtwData[i].idpasarela != null) {
                        ...
                        var sppe = oldGtwData[i].sppe ? oldGtwData[i].sppe : 0;
                        var dvrrp = oldGtwData[i].dvrrp ? oldGtwData[i].dvrrp : 2000;
            ...
            var insertNewGtwTfnoRes = async function /*insertNewGtwTfnoRes*/(callback) {
                ...
                for (var i = 0; i < oldGtwTfnoResData.length; i++) {
                    ...
                    /** 20200713. Resumen de los Valores...*/
                        var clave_registro = oldGtwTfnoResData[i].clave_registro == null ? "" : oldGtwTfnoResData[i].clave_registro;
                        var iDetLineaAB = oldGtwTfnoResData[i].iDetLineaAB ? oldGtwTfnoResData[i].iDetLineaAB : 0;
                        var iEnableNoED137 = oldGtwTfnoResData[i].iEnableNoED137 ? oldGtwTfnoResData[i].iEnableNoED137 : 0;
                        var itiporespuesta = oldGtwTfnoResData[i].itiporespuesta ? oldGtwTfnoResData[i].itiporespuesta : 0;
                        var additional_uri_remota = oldGtwTfnoResData[i].additional_uri_remota ? oldGtwTfnoResData[i].additional_uri_remota : "";
                        var additional_superv_options = oldGtwTfnoResData[i].additional_superv_options ? oldGtwTfnoResData[i].additional_superv_options : 0;
                        var additional_itiporespuesta = oldGtwTfnoResData[i].additional_itiporespuesta ? oldGtwTfnoResData[i].additional_itiporespuesta : 0;
                        ...
                        /** 20200710. NUevos Parámetros para lineas telefonicas */
                            'iDetLineaAB, iEnableNoED137, itiporespuesta, additional_uri_remota, additional_superv_options, additional_itiporespuesta' +
                            ') VALUES ' +
                            '(' + gtwId2Insert + ',' + oldGtwTfnoResData[i].fila + ',' + oldGtwTfnoResData[i].columna + '' +
                            ',"' + oldGtwTfnoResData[i].nombre + '",' + oldGtwTfnoResData[i].codec + ',"' + clave_registro + '"' +
                            ...
                            ',' + additional_superv_options +
                            ',' + additional_itiporespuesta +
                            ');';
                        logging.Info("copyConfiguration TfnoRes", idSourceConfig, i);
...
exports.postConfigurationFromJsonFile = function(idcfg, idSite, newCgwData, f) {
    ...
    logging.Info("postConfigurationFromJsonFile", idcfg, idSite);
    ...
    connection.connect(function(err) {
        if (err) {
            ...
            var insertGtwData = function /*insertGtwData*/(callback) {
                /** 20190219. Campos para la supervision de puerta de enlace */
                var sppe = newCgwData.general.SupervisionLanGW && newCgwData.general.SupervisionLanGW==1 ?
                (newCgwData.general.TmMaxSupervLanGW ? newCgwData.general.TmMaxSupervLanGW  : 1) : 0;
                var dvrrp = newCgwData.general.dvrrp ? newCgwData.general.dvrrp : 2000;
                ...
                if (flagResources.resTfno) {
                    for (var i = 0; i < newCgwData.recursos.length; i++) {
                        if (newCgwData.recursos[i].Radio_o_Telefonia == 2) {                            
                            var adGain = 0;
                            var daGain = 0;
                            /** 20190219. Campos para ats_user */
                            var ats_user = newCgwData.recursos[i].telefonia.ats_user ? newCgwData.recursos[i].telefonia.ats_user : "";
                            /** 20190219. Campos para DetInversionPol */
                            var DetInversionPol = newCgwData.recursos[i].telefonia.iDetInversionPol ? newCgwData.recursos[i].telefonia.iDetInversionPol : 0;
                            /** 20200710. NUevos Parámetros para lineas telefonicas */
                            var iDetLineaAB = newCgwData.recursos[i].telefonia.iDetLineaAB ? newCgwData.recursos[i].telefonia.iDetLineaAB : 0;
                            var iEnableNoED137 = newCgwData.recursos[i].telefonia.iEnableNoED137 ? newCgwData.recursos[i].telefonia.iEnableNoED137 : 0;
                            var itiporespuesta = newCgwData.recursos[i].telefonia.itiporespuesta ? newCgwData.recursos[i].telefonia.itiporespuesta : 0;
                            var additional_uri_remota = newCgwData.recursos[i].telefonia.additional_uri_remota ? newCgwData.recursos[i].telefonia.additional_uri_remota : "";
                            var additional_superv_options = newCgwData.recursos[i].telefonia.additional_superv_options ? newCgwData.recursos[i].telefonia.additional_superv_options : 0;
                            var additional_itiporespuesta = newCgwData.recursos[i].telefonia.additional_itiporespuesta ? newCgwData.recursos[i].telefonia.additional_itiporespuesta : 0;
                            ...
                            queryString = 'INSERT INTO recursos_telefono (pasarela_id,fila,columna,nombre,codec,' +
                                ...
                                'destino_test,supervisa_colateral,tiempo_supervision,duracion_tono_interrup,uri_telefonica, ats_user, det_inversion_pol, ' +
                                /** 20200710. NUevos Parámetros para lineas telefonicas */
                                'iDetLineaAB, iEnableNoED137, itiporespuesta, additional_uri_remota, additional_superv_options, additional_itiporespuesta' +
                                ') VALUES (' + newInsertCgwId + ',' + newCgwData.recursos[i].NumDispositivoSlot + ',' +
                                '' + newCgwData.recursos[i].SlotPasarela + ',"' + newCgwData.recursos[i].IdRecurso + '",' +
                                ...
                                ',' + additional_itiporespuesta +
                                ')';
                            logging.Info("postConfigurationFromJsonFile TfnoRes", idcfg, i);
                            //Falta tabla_bss_id
                            res = await ug5kdb.QueryMultiInsertSync(queryString);
                            if (res.error)
                                logging.Error(res.error);
